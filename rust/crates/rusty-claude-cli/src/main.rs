...
enum ModelSource {
    ...
    Flag,
    /// CLAW_MODEL / ANTHROPIC_MODEL environment variable (when no flag was passed).
    Env,
...
"#,
    ...
    #[test]
    fn resolve_repl_model_prefers_claw_model_over_anthropic_model() {
        let _guard = env_lock();
        let root = temp_dir();
        fs::create_dir_all(&root).expect("root dir");
        let config_home = root.join("config");
        ...
        std::env::set_var("CLAW_CONFIG_HOME", &config_home);
        std::env::set_var("ANTHROPIC_MODEL", "sonnet");
        std::env::set_var("CLAW_MODEL", "haiku");
