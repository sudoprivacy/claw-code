...
fn resumed_status_command_emits_structured_json_when_requested() {
    ...
    // Isolate from user home config to avoid loading real config files.
    let isolated_config_home = temp_dir.join("isolated-config");
    fs::create_dir_all(&isolated_config_home).expect("isolated config home");
    ...
    let output = run_claw_with_env(
        ...
        &[(
            "CLAW_CONFIG_HOME",
            isolated_config_home.to_str().expect("utf8 path"),
        )],
    );
